...
from main.helper.classes import ConfigHelper

####################################################################################
class Command(BaseCommand):
    help = 'generates .env.example file from configured environment variables in apps.py'
    file_intro = ('# This is an example .env file with following structure:\n'
                  '# ENV_VARNAME=[DEFAULT_VALUE|Nothing] # required? - comment'
                  '\n# first part is for internal things (i.e. services which run on internal docker containers)'
    ...
    def handle(self, *args, **options):
        ...
        _cfgs = [app_cfg for app_cfg in apps.get_app_configs() if issubclass(type(app_cfg), ConfigHelper)]
